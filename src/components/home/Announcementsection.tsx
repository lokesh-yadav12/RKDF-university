...
const announcements: Announcement[] = [
  {
    ...
    color: "from-blue-500 to-rose-200",
    gradient: "bg-gradient-to-br from-blue-50 to-rose-50"
  }
];
    ...
    <section className="relative py-12 overflow-hidden ">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="text-center mb-8">
          ...
          <a 
            href="/admission-form"
            className="inline-flex items-center space-x-3  text-black px-8 py-4 rounded-2xl mb-8 shadow-2xl hover:shadow-orange-500/50 transition-all duration-500 hover:scale-105 group relative overflow-hidden cursor-pointer ring-4 ring-blue-500/30 hover:ring-purple-400/60 hover:ring-3"
            onMouseEnter={() => setHoveredId(999)}
            onMouseLeave={() => setHoveredId(null)}
          >
            {/* Animated shine effect */}
            <div className="absolute inset-0 bg-black/20 transform -skew-x-12 -translate-x-full group-hover:translate-x-full transition-transform duration-1000"></div>
            ...
            <span className="font-bold text-lg tracking-wide relative z-10 underline decoration-2 underline-offset-4">Online Admission Form - 2023-24</span>
            <Star className="w-5 h-5 animate-spin-slow relative z-10" />
            <ExternalLink className={`w-5 h-5 relative z-10 transition-transform duration-300 ${hoveredId === 999 ? 'translate-x-1 -translate-y-1' : ''}`} />
          ...
          <div className="flex items-center justify-center space-x-3">
            ...
            <p className="text-lg text-gray-700 font-light">Stay informed with the latest news from RKDF University</p>
            <Sparkles className="w-5 h-5 text-yellow-400 animate-pulse" />
        ...
        <div className="mb-6">
          {announcements.map((announcement) => (
            ...
            >
              <div 
                className="relative backdrop-blur-xl bg-black/5 rounded-3xl p-4 shadow-2xl border border-black/10 hover:border-blue-500/50 transition-all duration-700 overflow-hidden hover:shadow-blue-500/20 hover:shadow-[0_0_50px_rgba(236,72,153,0.3)]"
              ...
              >
                ...
                <div className="relative z-10 flex items-start space-x-8">
                  ...
                  <div className="flex-1">
                    ...
                    <h3 className="text-2xl md:text-3xl  text-black mb-4 group-hover:bg-clip-text group-hover:bg-gradient-to-r group-hover:from-blue-700  transition-all duration-500">
                      {announcement.title}
                    </h3>
                    
                    <div className="inline-flex items-center space-x-3 px-6 py-3 bg-gradient-to-r from-pink-300/20 to-purple-300/20 backdrop-blur-sm rounded-xl border border-blue-500/30  transition-all duration-500 group-hover:shadow-lg group-hover:shadow-blue-500/20">
                      <span className="text-gray-600 font-semibold">Read More</span>
                      <ChevronRight className={`w-5 h-5 text-gray-400 transition-transform duration-500 ${hoveredId === announcement.id ? 'translate-x-2' : ''}`} />
                    </div>
                  </div>
                </div>
                ...
                {[...Array(5)].map((_, i) => (
                  <div
                    key={i}
                    className="absolute w-1 h-1 bg-pink-400 rounded-full opacity-0 group-hover:opacity-100 transition-opacity duration-500"
                    style={{
                      top: `${20 + i * 15}%`,
                      right: `${10 + i * 5}%`,
          ...
          {quickLinks.map((link, index) => (
            ...
            >
              <div className="relative h-full backdrop-blur-xl bg-black/5 rounded-2xl p-3 shadow-xl border border-black/10 hover:border-blue-500/50 transition-all duration-700 overflow-hidden group-hover:shadow-2xl group-hover:shadow-blue-500/20 transform hover:-translate-y-3 hover:scale-[1.02]">
                ...
                {/* Corner Accent */}
                <div className="absolute top-0 right-0 w-24 h-24 bg-gradient-to-br from-black/5 to-transparent rounded-bl-3xl"></div>

                <div className="relative z-10">
                  {/* Icon with Depth */}
                  <div className="relative mb-6 inline-block">
                    <div className={`absolute inset-0 bg-gradient-to-br ${link.color} rounded-xl blur-lg opacity-50 group-hover:opacity-100 transition-opacity duration-500`}></div>
                  ...
                  <div className="flex items-center space-x-2 text-blue-400 font-semibold group-hover:text-blue-600 transition-colors duration-300">
                    ...
                    <ExternalLink className={`w-4 h-4 transition-transform duration-500 ${hoveredId === link.id + 100 ? 'translate-x-2 -translate-y-1 rotate-12' : ''}`} />
                  </div>
                </div>
        ...
        <div className="mt-12 flex justify-center">
          <div className="inline-flex items-center space-x-4 backdrop-blur-xl bg-black/10 rounded-full px-10 py-5 shadow-2xl border border-black/20">
            ...
            <div className="relative">
              ...
              <div className="absolute inset-0 w-3 h-3 bg-green-500 rounded-full animate-ping" style={{ animationDelay: '1s' }}></div>
            </div>
          </div>
        </div>
      </div>
